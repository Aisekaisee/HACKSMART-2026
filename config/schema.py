...
class StationConfig:
    ...
    lat: float
    lon: float
    # Initial charged batteries (defaults to capacity)
    ...
    def __post_init__(self):
        ...
        # Validation
        assert self.tier in ["high", "medium",
                             "low"], f"Invalid tier: {self.tier}"
...
class OperationalConfig:
    ...
    max_wait_time: float = 15.0  # minutes - customer leaves if wait exceeds this
...
class BaselineConfig:
    ...
    # Duration in hours (if provided, overrides simulation_duration)
...
class ScenarioConfig:
    ...
    modify_stations: Dict[str, Dict] = field(
        default_factory=dict)  # {station_id: {attribute: value}}
