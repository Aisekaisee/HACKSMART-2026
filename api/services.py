...
def update_station(station_id: str, data: StationUpdate) -> Optional[Dict[str, Any]]:
    ...
    if not update_data:
        ...
        response = supabase.table("stations").select(
...
def run_simulation(
    ...
    for s in stations:
        ...
        # Use explicit tier if provided, otherwise compute from chargers
        chargers = s.get("chargers", 5)
        tier = s.get("tier")
        if not tier:
            # Compute tier based on chargers
            if chargers >= 10:
                tier = "high"
            elif chargers >= 5:
                tier = "medium"
    ...
    demand = DemandConfig(
        base_rates=demand_config.get(
            "base_rates", {"high": 30, "medium": 20, "low": 10}),
        time_multipliers=demand_config.get(
            "time_multipliers", {i: 1.0 for i in range(24)}),
    ...
    if interventions:
        ...
        for intervention in interventions:
            ...
            if int_type == "weather_demand":
                ...
                scenario_config.demand_multiplier = params.get(
                    "multiplier", 1.0)
            elif int_type == "modify_station":
                station_id = params.get("station_id")
                if station_id:
                    mods = {k: v for k, v in params.items() if k !=
                            "station_id"}
...
class SimulationService:
    ...
    def run_comparison(baseline: BaselineConfig, scenario: ScenarioConfig) -> ComparisonResponse:
        ...
        modified_schema = ScenarioApplicator.apply_scenario(
            base_schema, scen_schema)
    ...
    def _pydantic_to_schema_scenario(p_config: ScenarioConfig) -> SchemaScenarioConfig:
        ...
        add_stations = [StationConfig(**s.model_dump())
                        for s in p_config.add_stations]
        ...
        for s in baseline_kpis.stations:
            ...
            if s.avg_wait_time > 5.0:
                suggestions.append({
                    "type": "add_chargers",
                    "station_id": s.station_id,
                    "description": f"Long wait times at {s.station_id} ({s.avg_wait_time:.1f} min). Add more chargers to reduce wait.",
                    "priority": "high",
                    "action_payload": {"station_id": s.station_id, "add_chargers": 2}
                })
